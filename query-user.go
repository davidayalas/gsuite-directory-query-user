...
var GoogleClient, GoogleClientErr = CreateClient()
...
func CreateClient() (*admin.Service, error){
    ...
    if credentials == "" {
        return nil, &errorString{"no credentials"}
    }

    if impersonatedUser == "" {
        return nil, &errorString{"no admin impersonated user"}
    }
...
func GetUser(usuari string) (string, error){
    if GoogleClientErr == nil{
        ...
        if err != nil {
            ...
            return "", &errorString{"user not found"}
        }
    ...
    return "", &errorString{"Google Client not instantiated"}
}
